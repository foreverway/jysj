<template>
  <div class="main">
    <span>角色名称:</span>
    <el-input v-model="form.role_name" style="width:200px"></el-input>
    <br />
    <br />
    <br />
    ...
    <p style="margin-top:40px">
      <el-button type="primary" @click="postdata">编辑保存</el-button>
...
export default {
  ...
  methods: {
    ...
    getdata() {
          ...
          for (var i = 0; i < this.cityOptions.length; i++) {
            ...
            for (var z = 0; z < this.cityOptions[i].children.length; z++) {
              ...
              if (this.cityOptions[i].children[z].children) {
                for (
                  var y = 0;
                  y < this.cityOptions[i].children[z].children.length;
                  y++
                ) {
                  this.allinputid.push(
                    this.cityOptions[i].children[z].children[y].id
                  );
                }
              }
            }
      ...
      // 获取角色id
      this.$apis.menber.role_edit(this.form).then(res => {
        this.form.role_name = res.data.data.role.role_name;

        this.form.menu_id = res.data.data.menu_id.map(Number);
      });
    },
    postdata() {
      this.$apis.menber.role_edit_put(this.form).then(res => {
        if (res.data.code == 1) {
          this.$message({
            message: "编辑成功",
            type: "success"
