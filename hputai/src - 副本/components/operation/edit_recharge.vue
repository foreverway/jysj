<template>
 ...
 <el-select v-model="form.student_type" placeholder="调整学生类型 " v-if="ifBanzhuren=='班主任'"    filterable clearable @change="getdata">
      ...
      <el-form-item label="收款方式：" style="width:1000px" prop="collection_type" v-if="msg.data">
        <el-select
          clearable
      ...
      <el-form-item label="充值订单：" prop="collection_type" v-show="form.collection_type=='用户自行充值'" v-if="msg.data">
      ...
      <el-form-item label="实收人：" prop="in_people" v-if="msg.data">
        <el-select
          clearable
          style="width:200px"
          class="so_input"
      ...
      <el-form-item label="第二收款人" v-if="msg.data">
      ...
      <el-form-item label="报课项目：" prop="classproject" v-if="msg.data">
...
export default {
  ...
  methods: {
    ...
    getdata() {
      ...
      //   id: this.id
      // };
      this.$apis.common.recharge_edit({id:this.$route.query.id}).then(res => {
        if (res.data.code == 1) {
